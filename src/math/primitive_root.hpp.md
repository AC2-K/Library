...
data:
  ...
  bundledCode: "#line 2 \"src/math/dynamic_modint.hpp\"\n#include <cassert>\n#include\
    ...
    \nnamespace kyopro {\n\n/**\n * @brief \u539F\u59CB\u6839\n */\ntemplate <typename\
    \ T> T primitive_root(T p) {\n    if (p == 2) return 1;\n    std::vector<T> pf\
    \ = kyopro::rho::factorize(p - 1);\n    pf.erase(std::unique(pf.begin(), pf.end()),\
    \ pf.end());\n    for (T& q : pf) {\n        q = (p - 1) / q;\n    }\n    using\
    \ unsigned_t = std::make_unsigned_t<T>;\n    if (dynamic_modint<unsigned_t>::get_mod()\
    \ != p) {\n        dynamic_modint<unsigned_t>::set_mod(p);\n    }\n    xor_shift32\
    \ rng(2023);\n    while (1) {\n        dynamic_modint<unsigned_t> g(rng());\n\
    \        if (g.val() == 0) continue;\n        bool is_ok = true;\n\n        for\
    \ (auto q : pf) {\n            if (dynamic_modint<unsigned_t>(g).pow(q).val()\
    \ == 1) {\n                is_ok = false;\n                break;\n          \
    \  }\n        }\n        if (is_ok) {\n            return g.val();\n        }\n\
    \    }\n}\n};  // namespace kyopro\n"
  code: "#pragma once\n#include \"../math/dynamic_modint.hpp\"\n#include \"../math/rho.hpp\"\
    ...
    \ 2) return 1;\n    std::vector<T> pf = kyopro::rho::factorize(p - 1);\n    pf.erase(std::unique(pf.begin(),\
    \ pf.end()), pf.end());\n    for (T& q : pf) {\n        q = (p - 1) / q;\n   \
